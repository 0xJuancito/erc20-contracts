...
/**
 * @dev L2 token is a mintable token.
 *
 * At construction, the deployer of the contract is the only minter.
 */
contract L2 is ERC20Burnable {
    // Address of the old token contract
    IERC20 public legacyToken;

    // To save gas on transfer from allowance equal to the max uint256 will be
    // considered infinite.
    uint256 private constant INFINITY = type(uint256).max;

    constructor(IERC20 legacyToken_) public ERC20("Leverj Gluon", "L2") {
        require(
            address(legacyToken_) != address(0),
            "L2: legacy token is the zero address"
        );
    ...
    function migrate(address account, uint256 amount) public {
        ...
        _mint(account, amount * 1000000000);
    }
     ...
     *
     * Notes:
     * - this version of the function will consider an allowance equal to the
     * maximum value representable by uint256 as INFINITY, if this is the case,
     * the allowance will remain untouched and no {Approval} event will be emited.
     */
    function transferFrom(
        address sender,
        address recipient,
