...
abstract contract ElasticToken is IElasticToken, OwnableUpgradeable {
    ...
    function __ElasticToken_init(string memory _name, string memory _symbol, uint8 _decimals) public onlyInitializing {
    ...
    function _burn(address to, uint256 amount) internal validRecipient(to) {
        ...
        // Eliminate dust
        if (checkDust(_underlyingBalances[to])) {
            _underlyingBalances[to] = 0;
        }
    ...
    function transfer(address to, uint256 value) external override validRecipient(to) returns (bool) {
        ...
        // Eliminate dust
        if (checkDust(_underlyingBalances[msg.sender])) {
    ...
    function transferFrom(address from, address to, uint256 value) external override validRecipient(from) validRecipient(to) returns (bool) {
        ...
        // Eliminate dust
        if (checkDust(_underlyingBalances[from])) {
            _underlyingBalances[from] = 0;
        }
    ...
    */
    ...
    function valueToUnderlying(uint256 value) public override view returns (uint256) {
        // Note: rounding up (ceiling) to prevent value => underlying => value to get a lower value (breaks UniswapV3 minting)
        return (value * SCALING_FACTOR_DECIMALS - 1) / scalingFactor + 1;
    }

    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual {}

    function checkDust(uint256 underlyingValue) private returns (bool isDust) {
        if (underlyingToValue(underlyingValue) == 0 || underlyingValue <= 100) {
            initSupply -= underlyingValue;
            totalSupply = underlyingToValue(initSupply);
            isDust = true;
        }
    }
