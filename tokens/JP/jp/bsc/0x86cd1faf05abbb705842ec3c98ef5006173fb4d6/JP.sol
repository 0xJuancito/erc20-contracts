...
library Address{
 ...
 contract JP is Context, IERC20, Ownable {
    ...
    uint256 private initialsupply = 3_000_000_000;
    ...
    string private constant _name = "JP";
    ...
    constructor (address routerAddress, address _marketingWalletAddress, address _donationWalletAddress) {
        ...
        donationWallet = payable(_donationWalletAddress);
    ...
    function airdropTokens(address[] memory accounts, uint256[] memory amounts) external onlyOwner{
        ...
        for(uint256 i = 0; i < accounts.length; i++){
            _tokenTransfer(msg.sender, accounts[i], amounts[i] * 10**9 , false, false);
        }
    }
    ...
    function updateMarketingWallet(address updateMW) external onlyOwner{
        marketingWallet = payable(updateMW);
    }
    
    function updateDonationWallet(address updateDW) external onlyOwner{
        donationWallet = payable(updateDW);
    }
