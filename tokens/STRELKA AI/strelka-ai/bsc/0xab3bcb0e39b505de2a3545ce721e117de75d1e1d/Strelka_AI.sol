...
contract TaxHandler is ITaxHandler,Auth {
    ...
    function addLiquidity(
        ...
        uint256 bnbAmount = tokenAmount - halfTokenAmount;
        ...
        uint256 bnbBalance = bnbBalanceAfter - bnbBalanceBefore;

        token.approve(address(router), halfTokenAmount);
        router.addLiquidityETH{value: bnbBalance}(
            address(token),
            halfTokenAmount,
...
contract Strelka_AI is IERC20, Auth {
    address constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    
    string constant _name = "Strelka AI";
    string constant _symbol = "STRELKA AI";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(sender==address(pair)) { // When buying STRELKA in WBNB
            if(antiBotEnabled) {
