...
contract CentroFi is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public minimumAmountSwap = 10** 12;
    ...
    constructor() ERC20("CentroFi", "CENTRO") {
        address newOwner = 0xC70F7F4f6226AE33A53e663c1041A26bc3E9BeDb;
        _decimals = 12;
    
        uint256 totalSupply = 1e8 * (10**_decimals);
        ...
        totalTransferFees = rewardsTransferFee + marketingTransferFee + liquidityTransferFee;
        
    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(0xE420739A0aAD832aa578cc0aFcA4A48Bd8F31Aa4); // set as marketing wallet
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee) external onlyOwner {
        ...
        require(totalBuyFees + totalSellFees + totalTransferFees <= 20, "Must keep fees at 20% or less");
    }
    ...
    function updateTransferFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee) external onlyOwner {
        marketingTransferFee = _marketingFee;
        rewardsTransferFee = _rewardsFee;
        liquidityTransferFee = _liquidityFee;
        totalTransferFees = marketingTransferFee + rewardsTransferFee + liquidityTransferFee;
        require(totalBuyFees + totalSellFees + totalTransferFees <= 20, "Must keep fees at 20% or less");
    }
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 1000 * (10**12); //must hold tokens
    }
