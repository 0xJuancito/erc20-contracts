...
contract TSHPTokenImpl is TSHPToken,ERC20Burnable {
    function transferableBalanceOf(address holder) public view returns (uint256) {
        uint256 totalBalance = _balances[holder];
        if( timelockList[holder].length >0 ){
            for(uint i=0; i<timelockList[holder].length;i++){
                if(timelockList[holder][i]._releaseTime <= now) {
                    totalBalance = totalBalance.add( _unlockableBalance(holder,i) );
                }
            }
        }
        return totalBalance;
    ...
    function unlockableBalanceOf(address holder) public view returns (uint256) {
    ...
    function realBalanceOf(address holder) public view returns(uint256) {
        return _balances[holder];
    }
    
    // -2019. 10. 02
    function recall(address _holder, uint256 _amount) public onlyOwner returns (bool)
    {
        _autoUnlock(_holder);
        _balances[_holder] = _balances[_holder].sub(_amount);
        _balances[owner] = _balances[owner].add(_amount);
        emit Transfer(_holder, owner, _amount);
        
        return true;
    }
    
    function unlockAndRecall(address _holder, uint256 _index) public onlyOwner returns(bool)
    {
        //_autoUnlock(_holder);
        LockInfo storage lockinfo = timelockList[_holder][_index];
        uint256 releaseAmount = lockinfo._remainingAmount;
        _unlock(_holder, _index);

        _balances[_holder] = _balances[_holder].sub(releaseAmount);
        _balances[owner] = _balances[owner].add(releaseAmount);
        emit Transfer(_holder, owner, releaseAmount);
        
        return true;
    }
    
    function getLocklistLength(address _holder) public view returns(uint256) {
        return timelockList[_holder].length;
    }
}