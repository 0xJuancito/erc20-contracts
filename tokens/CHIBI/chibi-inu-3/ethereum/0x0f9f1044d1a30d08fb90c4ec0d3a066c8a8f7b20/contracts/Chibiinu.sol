...
contract Chibiv2 is ERC20, Ownable {
    ...
    uint256 public buyFees = uint256(0.007 * 1e18);
    uint256 public sellFees = uint256(0.007 * 1e18);
    ...
    constructor() ERC20("Chibi inu", "CHIBI") {
    ...
    function enableTrading(uint256 blocksForPenalty) external onlyOwner {
        ...
        function airdropTokens(address[] memory recipients, uint[] memory amounts) external onlyOwner {
        ...
        for (uint i = 0; i < recipients.length; i++) {
            ...
            // Convert the amount to uint256 (assuming 8 decimals)
            uint256 amountInWei = amounts[i] * 1e8;
            
            // Transfer tokens to the recipient
            _transfer(msg.sender, recipients[i], amountInWei);
        }
    }
