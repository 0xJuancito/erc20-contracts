...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
    ...
    function addTWAPSource(address newTWAPSourceDexPool_) external onlyOwner {
        require(_dexPoolsTWAPSources.add(newTWAPSourceDexPool_), "LoveToken: TWAP Source already stored.");
        emit TWAPSourceAdded(newTWAPSourceDexPool_);
    }

    function removeTWAPSource(address twapSourceToRemove_) external onlyOwner {
        require(_dexPoolsTWAPSources.remove(twapSourceToRemove_), "LoveToken: TWAP source not present.");
...
contract LoveToken is Divine {
    using SafeMath for uint256;

    constructor() Divine("Hunny Love Token", "LOVE", 9) {}
