...
contract Chainex is ERC20, Ownable {
    ...
    struct TokenDistributionForTax {
        ...
        uint64 tokensForLP;
        uint64 tokensForDev;
        uint64 tokensForTeam;
        uint64 tokensForRevShare;
        uint64 tokensForNftShare;
        bool gasSaver;
    }
    ...
    {   
        ...
        mktingAddress = 0xfC96d2B93BF6e29c0324AeA2193B1aaba90aC561; // update
        devAddress = 0xdda1195d5B3f3ed8b299068068410c3738b04B25; // update
    ...
    function handleTax(address from, address to, uint256 amount) internal returns (uint256){
        ...
        if(taxes.totalTax > 0){
            ...
            tokensForTaxUpdate.tokensForLP += uint64(tax * taxes.lpTax / taxes.totalTax / 1e12);
            tokensForTaxUpdate.tokensForMkting += uint64(tax * taxes.mktingTax / taxes.totalTax / 1e12);
            tokensForTaxUpdate.tokensForNftShare += uint64(tax * taxes.nftShareTax / taxes.totalTax / 1e12);
            tokensForTaxUpdate.tokensForDev += uint64(tax * taxes.devTax / taxes.totalTax / 1e12);
            tokensForTaxUpdate.tokensForTeam += uint64(tax * taxes.teamTax / taxes.totalTax / 1e12);
            tokensForTaxUpdate.tokensForRevShare += uint64(tax * taxes.revShareTax / taxes.totalTax / 1e12);
    ...
    function newSellTaxes(uint24 _mktingTax, uint24 _lpTax, uint24 _devTax, uint24 _revShareTax, uint24 _nftShareTax, uint24 _teamTax) external onlyOwner {
        Tax memory taxes;
    ...
    function newBuyTaxes(uint24 _mktingTax, uint24 _lpTax, uint24 _devTax, uint24 _revShareTax, uint24 _nftShareTax, uint24 _teamTax) external onlyOwner {
        Tax memory taxes;
        taxes.mktingTax = _mktingTax;
        taxes.lpTax = _lpTax;
        taxes.devTax = _devTax;
