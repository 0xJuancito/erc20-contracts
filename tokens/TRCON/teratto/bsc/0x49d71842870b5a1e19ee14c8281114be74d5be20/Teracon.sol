...
contract Lockable is Context {
     ...
     * @param period Vesting lock period.
    ...
    /**
     * @dev Remove Vesting lock, only locker can remove
...
/**
 * @dev Contract for Teracon token
 */
contract Teracon is Pausable, Ownable, Burnable, Lockable, ERC20 {
    ...
    function _beforeTokenTransfer(
        ...
        require(balanceOf(from) - getTimeLockedAmount(from) - getVestingLockedAmount(from) >= amount, "Lockable: token transfer from time locked account");
    }
    ...
    /**
     * @dev only locker can remove Vesting lock
     */
    // function removeVestingLock(address account) public onlyOwner whenNotPaused {
