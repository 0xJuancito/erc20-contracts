...
// File contracts/tokens/Fira.sol
 ...
 * @title Fira
 */
contract Fira is ERC20, ERC20Burnable, Ownable, Authorizable {
    ...
    constructor(uint256 cap_, uint256 amount_) public ERC20("Fira", "FIRA") {
        _cap = cap_;
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by authorized addresses.
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "Fira::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "Fira::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "Fira::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "Fira::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "Fira::_writeCheckpoint: block number exceeds 32 bits"
        );
