...
/*
    ...
    Website - https://www.digifundtoken.com
    Twitter - https://twitter.com/digifund_token
    Telegram - https://t.me/digifund_token

    Welcome to DigiFund, a decentralised investment fund where your percentage of token supply is reflected as your percent of ownership of total project reserves in the treasury fund.
...
contract DFUND is Context, IERC20, Ownable {
    ...
    string private _name = "DigiFund";
    string private _symbol = "DFUND";
    ...
    uint256 public liquidityPart = 10;

    uint256 public snipersCaught;
    uint256 private liqAddBlock;
    uint256 private snipeBlockAmt;
    
    address public uniswapV2Pair;
    address payable public digiFundWallet = 0xdF487A2DAfB8c3116d4dc2299365933FbF9b27ED;
    address payable public digiMarketingWallet = 0x7342d9A4F7cC68106A9B10E8771172710145157f;
    ...
    event SetBurnFee(uint256 newBurnFee);
    event SetTaxFee(uint256 newTaxFee);
    event SetLiquidityFee(uint256 newLiquidityFee);    
    event SetDFUNDWallet(address newTreasuryWallet, address digiMarketingWallet);
    event DFUNDBurnPayoutRequest(uint256 rBurn, address, uint256 percentBurnt);
    ...
    function getCirculatingSupply() public view returns (uint256) {
        ...
        return totalSupply().sub(burnAddressTokens);
    }
    ...
    function _transfer(
        ...
        if (!_hasLiqBeenAdded) {
            ...
            if (liqAddBlock > 0 
                ...
                if (block.number - liqAddBlock < snipeBlockAmt) {
                    _isSniper[to] = true;
                    _isExcluded[to] = true;
                    _excluded.push(to);
                    snipersCaught ++;
        ...
        if(from == uniswapV2Pair && !inSwapAndLiquify && !_isExcludedFromFee[to]) {
            takeFee = true;
        }

        if (to == uniswapV2Pair && !inSwapAndLiquify && !_isExcludedFromFee[from]) {
            takeFee = true;
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 halfOfLiquify = contractTokenBalance.div(liquidityPart);
        uint256 otherHalfOfLiquify = contractTokenBalance.div(liquidityPart);
    ...
    function sendETH(uint256 amount) private { 
        ...
        digiFundWallet.transfer(taxETH.mul(80).div(100)); 
        digiMarketingWallet.transfer(taxETH.mul(20).div(100)); 
    }
    ...
    function setFees(uint256 BurnFee, uint256 TaxFee, uint256 LiquidityFee) external onlyOwner() {
        require((BurnFee + TaxFee + LiquidityFee) <= 600, "Fees too high, must remain less than 6%");
        _burnFee = BurnFee;
        _taxFee = TaxFee;
        _liquidityFee = LiquidityFee;

        emit SetBurnFee(BurnFee);
        emit SetTaxFee(TaxFee);
    ...
    function setDigiFundWallet(address payable _digiFundWallet, address payable _digiMarketingWallet) external onlyOwner() {
        digiFundWallet = _digiFundWallet;
        digiMarketingWallet = _digiMarketingWallet;
        emit SetDFUNDWallet(_digiFundWallet, _digiMarketingWallet);
    }
    ...
    function DFUNDBurnPayout(uint256 rBurn) external lockTheBurn {
        require(rBurn > 0, "Not enough");

        uint256 rBurnX = rBurn * 10**9;
        require(balanceOf(msg.sender) > rBurnX, "Not enough");

        _rOwned[msg.sender] -= rBurnX;
        _tBurnTotal = _tBurnTotal.add(rBurnX);
        _tTotal = _tTotal.sub(rBurnX);

        emit DFUNDBurnPayoutRequest(rBurnX, msg.sender, _tTotal);
    }
