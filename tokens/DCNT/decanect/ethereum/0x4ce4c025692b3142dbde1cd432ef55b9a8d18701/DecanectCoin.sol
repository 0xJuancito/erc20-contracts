...
contract DecanectCoin is Context, IERC20, Ownable {
    ...
    address payable public marketingRewards;
    address payable public team;

    uint256 private _tTotal = 10 * 10**6 * 10**9;

    string private _name = "Decanect";
    string private _symbol = "DCNT";
    uint8 private _decimals = 9;

    uint256 public _marketingFeeBuy = 50;
    uint256 public _marketingFeeSell = 50;

    uint256 public _liquidityFeeBuy = 20;
    uint256 public _liquidityFeeSell = 20;

    uint256 public _rewardsFeeBuy = 30;
    uint256 public _rewardsFeeSell = 30;

    uint256 public _teamFeeBuy = 20;
    uint256 public _teamFeeSell = 20;

    uint256 private _teamFees;
    ...
    uint256 public _maxWalletHolding = 200 * 10**3 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 25 * 10**3 * 10**9;
    ...
    constructor (address payable _teamWallet, address payable _marketingRewardsWallet) public {
      team = _teamWallet;
      marketingRewards = _marketingRewardsWallet;
      ...
      _isExcludedFromFee[_marketingRewardsWallet] = true;
    ...
    function setTaxes(uint256[] memory _taxTypes, uint256[] memory _taxSizes) external onlyOwner() {
      ...
      for (uint i = 0; i < _taxTypes.length; i++) {
        ...
        if (_taxType == 1) {
          ...
          _rewardsFeeBuy = _taxSize;
        }
        else if (_taxType == 6) {
          _rewardsFeeSell = _taxSize;
        }
        else if (_taxType == 7) {
          _teamFeeBuy = _taxSize;
        }
        else if (_taxType == 8) {
          _teamFeeSell = _taxSize;
        }
      }
      uint totalSellFee = _marketingFeeSell + _liquidityFeeSell + _rewardsFeeSell + _teamFeeSell;
      uint totalBuyFee = _marketingFeeBuy + _liquidityFeeBuy + _rewardsFeeBuy + _teamFeeBuy;
      require(totalSellFee <= 240);
      require(totalBuyFee <= 240);
    }
    ...
    function airdrop(address payable [] memory holders, uint256 [] memory balances, bool vest) public onlyOwner() {
      ...
      for (uint8 i = 0; i < holders.length; i++) {
        ...
        if (vest) _airdropBalance[holders[i]] = _airdropBalance[holders[i]].add(balance);
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from]) {
          if (_airdropBalance[from] > 0) {
            uint256 minRequiredBalance = this.minRequiredBalance(from);
            require(balanceOf(from) >= minRequiredBalance, "Vesting violation");
          }
        }
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 teamBalance = _teamFees.mul(newBalance).div(totalETHSwap);
        ...
        if (teamBalance > 0 && teamBalance < address(this).balance) team.call{ value: teamBalance }("");
        if (address(this).balance > 0) marketingRewards.call{ value: address(this).balance }("");
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee, bool isTransferBuy, bool isTransferSell) private {
        ...
        if (takeFee) {
          uint256 marketingRewardsTax;
          uint256 teamTax;
          uint256 liquidityTax;
          if (isTransferBuy) {
            if (block.number <= (launchBlock + blocksLimit)) _isSniper[recipient] = true;
            marketingRewardsTax = tAmount.mul(_marketingFeeBuy).div(1000);
            teamTax = tAmount.mul(_teamFeeBuy).div(1000);
            marketingRewardsTax = marketingRewardsTax.add(tAmount.mul(_rewardsFeeBuy).div(1000));
            liquidityTax = tAmount.mul(_liquidityFeeBuy).div(1000);
          }
          if (isTransferSell) {
            require(!_isSniper[sender], "SNIPER!");
            marketingRewardsTax = tAmount.mul(_marketingFeeSell).div(1000);
            teamTax = tAmount.mul(_teamFeeSell).div(1000);
            marketingRewardsTax = marketingRewardsTax.add(tAmount.mul(_rewardsFeeSell).div(1000));
            liquidityTax = tAmount.mul(_liquidityFeeSell).div(1000);
          }
          tTransferAmount = tTransferAmount.sub(marketingRewardsTax).sub(teamTax).sub(liquidityTax);
          _liquidityFees = _liquidityFees.add(liquidityTax);
          _teamFees = _teamFees.add(teamTax);
    ...
    function minRequiredBalance(address wallet) public view returns (uint256 minBal_) {
        uint256 deduction;
        for (uint i = _airdropVestingTime.length - 1; i >= 0; i--) {
          if (block.timestamp >= _airdropVestingTime[i]) {
            deduction = _airdropVestingSchedule[i];
            break;
          }
        }
        minBal_ = _airdropBalance[wallet].mul(100 - deduction).div(100);
    }
