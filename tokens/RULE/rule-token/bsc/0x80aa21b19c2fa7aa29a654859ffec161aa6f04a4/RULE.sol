...
contract RULE is ERC20, Ownable {
    ...
    constructor () ERC20("RULE Token", "RULE") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        marketingWallet = 0x15755DDDc99020BDc436036730cD16b0F1F42Cb8;
    ...
    function updateBuyFees(uint256 _liquidityFeeOnBuy, uint256 _marketingFeeOnBuy) external onlyOwner {
        ...
        require(_totalFeesOnBuy <= 12, "Total Fees cannot exceed the maximum");
    ...
    function updateSellFees(uint256 _liquidityFeeOnSell, uint256 _marketingFeeOnSell) external onlyOwner {
        ...
        require(_totalFeesOnSell <= 12, "Total Fees cannot exceed the maximum");
    ...
    function updateTaxesEnabled(bool newState) external onlyOwner {        
        require(!tradingEnabled || !newState, "Taxes can't be re-enabled after trading has started");
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        uint256 _totalFees;
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || swapping || !taxesEnabled) {
            _totalFees = 0;
