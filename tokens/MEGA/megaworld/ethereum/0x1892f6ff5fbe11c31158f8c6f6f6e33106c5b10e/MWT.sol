...
contract MWT is ERC20, Manageable {
    mapping(address => bool) public operators;

    constructor() ERC20("MegaWorld $MEGA Token", "MEGA") public {}

    function setOperator(address payable _operator) public onlyManager {
        operators[_operator] = true;
    }

    function removeOperator(address payable _operator) public onlyManager {
        resetOperatorAllowance(_operator);
    ...
    function mintTo(address payable _to, uint _amount) public onlyOwner {
        _mint(_to, _amount);
    }

    function increaseOperatorAllowance(address payable _address, uint _allowance) public onlyManager {
        require(operators[_address], "Only for operators");

        _approve(address(this), _address, _allowance);
    }

    function resetOperatorAllowance(address payable _address) public onlyManager {
        require(operators[_address], "Only for operators");
        _approve(address(this), _address, 0);
    }
