...
contract Rewardz is ERC20, Ownable {

    uint256 public maxBuyAmount;
    uint256 public maxSellAmount;
    uint256 public maxWalletAmount;

    GutterRouter public raynRouter;
    ...
    constructor() ERC20("Rewardz Automated Yield Network", "RAYN") {

        address newOwner = msg.sender;

        GutterRouter _raynRouter = GutterRouter(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        raynRouter = _raynRouter;

        lpPair = GutterFactory(_raynRouter.factory()).createPair(address(this), _raynRouter.WETH());
    ...
    function startTrading(uint256 blockJump) external onlyOwner {
        require(!tradingActive, "Cannot re-disable trading");
        tradingActive = true;
        swapEnabled = true;
        tradingActiveBlock = block.number;
        blockForPenaltyEnd = tradingActiveBlock + blockJump;
        emit EnabledTrading();
    }
    ...
    function raynDrop(address[] memory wallets, uint256[] memory amountsInTokens) external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(blockForPenaltyEnd > 0){
            require(!boughtEarly[from] || to == owner() || to == address(0xdead), "Bots cannot transfer tokens in or out except to owner/dead address!");
        }

        if(limitsInEffect){
            if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && !_isExcludedFromFees[from] && !_isExcludedFromFees[to]){
                if (transferDelayEnabled){
                    if (to != address(raynRouter) && to != address(lpPair)){
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = raynRouter.WETH();
        _approve(address(this), address(raynRouter), tokenAmount);
        raynRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
            address(this),
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        _approve(address(this), address(raynRouter), tokenAmount);
        raynRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
            0,
            0,
    ...
    function buyBackTokens(uint256 amountInWei) external onlyOwner {
        ...
        address[] memory path = new address[](2);
        path[0] = raynRouter.WETH();
        path[1] = address(this);

        raynRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amountInWei}(
            0,
            path,
            address(0xdead),
            block.timestamp
