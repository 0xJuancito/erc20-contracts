...
contract _0xBlack is ERC20, ERC20Burnable, Ownable, TokenRecover, Initializable {
    
    mapping (address => bool) public blacklisted;

    uint16 public swapThresholdRatio;
    
    uint256 private _rewardpoolPending;
    uint256 private _liquidityPending;

    address public rewardpoolAddress;
    uint16[3] public rewardpoolFees;
    ...
    event rewardpoolAddressUpdated(address rewardpoolAddress);
    event rewardpoolFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event rewardpoolFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20(unicode"0xBlack", unicode"0XB") 
    {
        address supplyRecipient = 0x6237A79B60A6238E40AcFd9Fe324E9FC6ccd123a;
        
        updateSwapThreshold(50);

        rewardpoolAddressSetup(0xC985ecC62291a40De607cd8D6D98E599F7d0Bcd1);
        rewardpoolFeesSetup(250, 0, 0);

        lpTokensReceiverSetup(0xC985ecC62291a40De607cd8D6D98E599F7d0Bcd1);
        ...
        _transferOwnership(0x6237A79B60A6238E40AcFd9Fe324E9FC6ccd123a);
    }
    ...
    function getAllPending() public view returns (uint256) {
        return 0 + _rewardpoolPending + _liquidityPending;
    }

    function rewardpoolAddressSetup(address _newAddress) public onlyOwner {
        require(_newAddress != address(0), "TaxesDefaultRouterWallet: Wallet tax recipient cannot be a 0x0 address");

        rewardpoolAddress = _newAddress;
        excludeFromFees(_newAddress, true);

        emit rewardpoolAddressUpdated(_newAddress);
    }

    function rewardpoolFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        totalFees[0] = totalFees[0] - rewardpoolFees[0] + _buyFee;
        totalFees[1] = totalFees[1] - rewardpoolFees[1] + _sellFee;
        totalFees[2] = totalFees[2] - rewardpoolFees[2] + _transferFee;
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        rewardpoolFees = [_buyFee, _sellFee, _transferFee];

        emit rewardpoolFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                _rewardpoolPending += fees * rewardpoolFees[txType] / totalFees[txType];
        ...
        if (!_swapping && !AMMPairs[from] && from != address(routerV2) && canSwap) {
            ...
            if (false || _rewardpoolPending > 0) {
                uint256 token2Swap = 0 + _rewardpoolPending;
                bool success = false;

                _swapTokensForCoin(token2Swap);
                uint256 coinsReceived = address(this).balance;
                
                uint256 rewardpoolPortion = coinsReceived * _rewardpoolPending / token2Swap;
                if (rewardpoolPortion > 0) {
                    success = payable(rewardpoolAddress).send(rewardpoolPortion);
                    if (success) {
                        emit rewardpoolFeeSent(rewardpoolAddress, rewardpoolPortion);
                    }
                }
                _rewardpoolPending = 0;
