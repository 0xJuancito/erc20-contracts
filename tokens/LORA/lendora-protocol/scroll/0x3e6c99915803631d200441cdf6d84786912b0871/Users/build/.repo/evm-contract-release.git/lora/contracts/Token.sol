...
contract Token is Ownable, ERC20Permit {
    ...
    constructor(string memory name_, string memory symbol_) Ownable() ERC20(name_,symbol_) ERC20Permit(name_) {
        teamLockToken = new TokenVesting(msg.sender, block.timestamp + 180 days, 0 days, 730 days);
        marketLockToken = new TokenVesting(msg.sender, block.timestamp + 180 days, 0 days, 355 days);

        _mint(address(teamLockToken), 1_000_000 * 1e18);
        _mint(address(marketLockToken), 200_000 * 1e18);

        // Initial Liquidity
        _mint(msg.sender, 500_000 * 1e18);
        // Airdrop
        _mint(msg.sender, 300_000 * 1e18);
        // Protocol Growth
