...
contract BVT_Token is ERC20, Ownable {
    ...
    function setWhiteListBatch(address[] memory permits, bool b) public onlyOwner returns (bool) {
        for(uint i = 0; i < permits.length;i++){
            whiteList[permits[i]] = b;
        }
        return true;
    }
    ...
    function safePullMain(address wallet) external onlyOwner{
