...
contract Magik is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 MAGIK-WFTM LP -> MAGIK pool
    uint256 public constant INITIAL_MAGIK_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the MAGIK ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MAGIK", "MAGIK") {
        // Mints 1 MAGIK to contract creator for initial pool setup
    ...
    function _getMagikPrice() internal view returns (uint256 _magikPrice) {
        try IOracle(magikOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Magik: failed to fetch MAGIK price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _magikPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_magikPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMagikOracle(address _magikOracle) public onlyOperatorOrTaxOffice {
        require(_magikOracle != address(0), "oracle address cannot be 0 address");
        magikOracle = _magikOracle;
    }
    ...
    /**
     * @notice Operator mints MAGIK to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MAGIK to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMagikPrice = _getMagikPrice();
            currentTaxRate = _updateTaxRate(currentMagikPrice);
            if (currentMagikPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_magikPool != address(0), "!_magikPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_magikPool, INITIAL_MAGIK_POOL_DISTRIBUTION);
