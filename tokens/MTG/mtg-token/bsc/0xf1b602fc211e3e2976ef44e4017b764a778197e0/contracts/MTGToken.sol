...
contract MTGToken is ERC20Burnable, Ownable {
    ...
    event IsExcludedFromFees(address targetAddress, bool state);
    ...
    function excludeFromFees(address targetAddress, bool state) external onlyOwner {
        isExcludedFromFees[targetAddress] = state;
        emit IsExcludedFromFees(targetAddress, state);
    }
    ...
    function _takeFee(address sender, address recipient, uint256 amount) internal returns (uint256 totalFee) {
        if (isExcludedFromFees[sender] || isExcludedFromFees[recipient] || _processingFee) {
            return 0;
        }
