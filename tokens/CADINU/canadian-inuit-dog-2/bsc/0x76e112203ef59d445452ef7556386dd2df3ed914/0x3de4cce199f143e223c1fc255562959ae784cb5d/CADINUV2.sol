...
// Token Contract
contract CADINUV2 is Initializable, ContextUpgradeable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(address => bool) private _isExcludedFromFeeInSend;
    mapping(address => bool) private _isExcludedFromFeeInReceive;
    ...
    uint256 private newTokenConvertMul;
    uint256 private newTokenConvertDiv;
    ...
    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "Cadinu: Invalid tier index");
        _;
    }
    ...
    function initialize() initializer public {
        _name = "Canadian Inuit Dog V2";
        _symbol = "CADINU";
        _decimals = 18;
        _tTotal = (10 ** 12) * (10 **_decimals);
        _rTotal = (MAX - (MAX % _tTotal));
        _maxFee = 10000000 * 10 ** _decimals;
    ...
    function __Cadinu_tiers_init() internal initializer {
        _defaultFees = _addTier(0, 500, 500, 0, 0, address(0), address(1));
    ...
    function excludeFromFeeInSend(address account) public onlyOwner {
        _isExcludedFromFeeInSend[account] = true;
    }

    function includeInFeeInSend(address account) public onlyOwner {
        _isExcludedFromFeeInSend[account] = false;
    }

    function excludeFromFeeInReceive(address account) public onlyOwner {
        _isExcludedFromFeeInReceive[account] = true;
    }

    function includeInFeeInReceive(address account) public onlyOwner {
        _isExcludedFromFeeInReceive[account] = false;
    }
    ...
    function whitelistAddress(address _account, uint256 _tierIndex)
        ...
        preventBlacklisted(_account, "Cadinu: Selected account is in blacklist")
    {
        require(_account != address(0), "Cadinu: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner {
        require(_account != address(0), "Cadinu: Invalid address");
        require(_accountsTier[_account] > 0, "Cadinu: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "Cadinu: Fees exceeded max limitation");
        return _tier;
    }
    ...
    function setMaxFee (uint256 maxFee) public onlyOwner{
        _maxFee = maxFee;
    }
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem)
        ...
        checkTierIndex(_tierIndex)
    {
        require(_ecoSystem != address(0), "Cadinu: Address Zero is not allowed");
        _excludeFromReward(_ecoSystem);
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner checkTierIndex(_tierIndex) {
        require(_owner != address(0), "Cadinu: Address Zero is not allowed");
    ...
    //to receive BNB from uniswapV2Router (pancakeswap) when swapping
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tTransferFee, uint256 currentRate) private pure
    ...
    function isExcludedFromFeeInSend(address account) public view returns (bool) {
        return _isExcludedFromFeeInSend[account];
    }

    function isExcludedFromFeeInReceive(address account) public view returns (bool) {
        return _isExcludedFromFeeInReceive[account];
    }
    ...
    function _approve(address owner, address spender, uint256 amount)
        internal override 
        preventBlacklisted(owner, "Cadinu: Owner address is blacklisted")
        preventBlacklisted(spender, "Cadinu: Spender address is blacklisted")
    {
    ...
    function _transfer(address from, address to, uint256 amount) 
        internal override
        preventBlacklisted(_msgSender(), "Cadinu: Address is blacklisted")
        preventBlacklisted(from, "Cadinu: From address is blacklisted")
        preventBlacklisted(to, "Cadinu: To address is blacklisted")
        isRouter(_msgSender())
    {
        ...
        if (_isExcludedFromFeeInSend[from]) {
            takeFee = false;
        }

        if (_isExcludedFromFeeInReceive[to]) {
            takeFee = false;
        }
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        ...
        uint256 initialBalance = ICadinu(address(this)).balanceOf(msg.sender);
        // swap BNB for Tokens
        swapBnbForTokens(half);

        // how much BNB did we just swap into?
        uint256 newBalance = ICadinu(address(this)).balanceOf(msg.sender);
        uint256 swapeedToken = newBalance - initialBalance;

        _approve(msg.sender, address(this), swapeedToken);
        ICadinu(address(this)).transferFrom(msg.sender, address(this), swapeedToken);
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        if (_token == address(this)){
            _tokenTransfer(address(this), _msgSender(), _amount, 0, false);
        }else{
            ERC20Upgradeable(_token).transfer(msg.sender, _amount);
        }  
    }
    ...
    function setMigrationSetting(address _migration, uint256 _newTokenConvertMul, uint256 _newTokenConvertDiv) public onlyOwner {
        migration = _migration;
        newTokenConvertMul = _newTokenConvertMul;
        newTokenConvertDiv = _newTokenConvertDiv;
    }
    ...
    function migrate(uint256 amount)
        external preventBlacklisted(_msgSender(), "Cadinu: Migrated account is blacklisted"){
        require(migration != address(0), "Cadinu: Migration is not started");
        ERC20Upgradeable(migration).transferFrom(_msgSender(), owner(), amount);
        uint256 newAmount;
        newAmount = (amount * newTokenConvertMul) / newTokenConvertDiv;
        _tokenTransfer(owner(), _msgSender(), newAmount, 0, false);
    }
}