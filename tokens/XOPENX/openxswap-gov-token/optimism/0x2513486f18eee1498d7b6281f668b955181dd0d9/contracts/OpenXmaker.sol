...
contract OpenXMaker {
    ...
    constructor (address _factory, address _openx, address _weth, address _bar) public {
    ...
    function burnX() public {
        uint256 balance = IERC20(openx).balanceOf(address(this));
        IERC20BURNABLE(openx).burn(balance);
    }
    ...
    function _convertStep(address token0, address token1, uint256 amount0, uint256 amount1) internal returns(uint256 openXOut) {
        ...
        if (token0 == token1) {
            ...
            if (token0 == openx) {
                IERC20BURNABLE(openx).burn(amount.div(3).mul(2)); //burn
                IERC20(openx).transfer(bar, amount.div(3)); //send to bar
                openXOut = amount;
            } else if (token0 == weth) {
            ...
            IERC20BURNABLE(openx).burn(amount0.div(3).mul(2)); //BURN
            IERC20(openx).transfer(bar, amount0.div(3)); //BAR

            openXOut = _toOPENX(token1, amount1).add(amount0);
        } else if (token1 == openx) { // eg. USDT - SUSHI
            IERC20BURNABLE(openx).burn(amount1.div(3).mul(2)); //BURN
            IERC20(openx).transfer(bar, amount1.div(3)); //BAR
    ...
    function _toOPENX(address token, uint256 amountIn) internal returns(uint256 amountOut) {
        ...
        IERC20BURNABLE(openx).burn(amountOut.div(3).mul(2)); //BAR
        IERC20(openx).transfer(bar, amountOut.div(3));
    }
}
