...
contract MOVEAPP is Ownable, ERC20 {
  address constant DEAD = 0x000000000000000000000000000000000000dEaD;
  address constant ZERO = 0x0000000000000000000000000000000000000000;
  address constant USDT = 0x55d398326f99059fF775485246999027B3197955;
  address constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

  string constant _name = "MoveApp";
  ...
  // Fees
  uint256 public feeDevelopmentWallet = 200; // 2%
  uint256 public feeDevelopmentWallet2 = 200; // 2%
  uint256 public feeDevelopmentWallet3 = 200; // 2%
  ...
  function getFeeTotal() public view returns(uint256) { return feeDevelopmentWallet + feeDevelopmentWallet2 + feeDevelopmentWallet3; }
  ...
  function setFeesForDevelopmentWallets(uint256 fee1, uint256 fee2, uint256 fee3) external onlyOwner {
    require(fee1 + fee2 + fee3 <= 600, "Total fees cannot exceed 6%");

    feeDevelopmentWallet = fee1;
    feeDevelopmentWallet2 = fee2;
    feeDevelopmentWallet3 = fee3;
}
  ...
  constructor()ERC20(_name, _symbol) {
    ...
    //Owner wallet
    address ownerWallet = 0x2d3fF4dDE10B466a45f419836B836fD23B68048a ;
    exemptFee[ownerWallet] = true;

    developingWallet = 0x66009BA1D040997b3B738e38C1484e2F18d7EdF4;
    developingWallet2 = 0xb06Fdc0518F2dCFFdEa1Ec65512f9182CDf326Da;
    developingWallet3 = 0xa28f832b068f4638bb408B7256A57208308Cda4B;
...
function _transfer(address sender, address recipient, uint256 amount) internal override {
    ...
    require(tradingEnabled || exemptFee[sender] || exemptFeeReceiver[recipient], "Trading is currently disabled");
    ...
    // Check if the transfer is a sale (sender is the token contract and recipient is the liquidity pool)
    if (recipient == liquidityPool) {
        if (!exemptFee[sender] && !exemptFeeReceiver[recipient]) {
            feeAmount = (getFeeTotal() * amount) / 10000;
 ...
 function exchangeFeeParts(uint256 incomingFeeTokenAmount) private returns (bool){
    ...
    // Deep Stack problem avoid
    {
      ...
      if (feeDevelopmentWallet > 0) tokenTransfer(usdtAddress, developingWallet, (usdtAmount * feeDevelopmentWallet) / totalFee);
      if (feeDevelopmentWallet2 > 0) tokenTransfer(usdtAddress, developingWallet2, (usdtAmount * feeDevelopmentWallet2) / totalFee);
      if (feeDevelopmentWallet3 > 0) tokenTransfer(usdtAddress, developingWallet3, (usdtAmount * feeDevelopmentWallet3) / totalFee);
    }
    return true;
  }
