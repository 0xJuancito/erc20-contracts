 ...
 */
contract DecentralizedAutonomousTrust is ERC20, ERC20Detailed {
  ...
  event Buy(address indexed _from, address indexed _to, uint _currencyValue, uint _fairValue);
  event Sell(address indexed _from, address indexed _to, uint _currencyValue, uint _fairValue);
  event Burn(address indexed _from, uint _fairValue);
  event Pay(address indexed _from, address indexed _to, uint _currencyValue, uint _fairValue);
  event Close(uint _exitFee);
  event StateChange(uint _previousState, uint _newState);
  event UpdateConfig(
  ...
  modifier onlyControl() {
    require(msg.sender == control, 'DAT: ONLY_CONTROL');
    _;
  }
  ...
  function _detectTransferRestriction(address _from, address _to, uint _value) private view returns (uint) {
    if (address(whitelist) != address(0)) {
      // This is not set for the minting of initialReserve
  ...
  function _transfer(address _from, address _to, uint _amount) internal authorizeTransfer(_from, _to, _amount, false) {
  ...
  function _burn(
    ...
    uint _amount,
    bool _isSell
  ) internal authorizeTransfer(_from, address(0), _amount, _isSell) {
  ...
  function _mint(address _to, uint _quantity) internal authorizeTransfer(address(0), _to, _quantity, false) {
  ...
  function _collectInvestment(uint _quantityToInvest, uint _msgValue, bool _refundRemainder) private {
    if (address(currency) == address(0)) {
      // currency is ETH
      if (_refundRemainder) {
  ...
  function _transferCurrency(address payable _to, uint _amount) private {
    if (_amount > 0) {
      if (address(currency) == address(0)) {
  ...
  function getChainId() private pure returns (uint id) {
  ...
  function _distributeInvestment(uint _value) private {
  ...
  function estimateBuyValue(uint _currencyValue) public view returns (uint) {
    ...
    if (state == STATE_INIT) {
      ...
      // impl: (g n (g + r - t))/(2 d)
      uint max = BigDiv.bigDiv2x1(initGoal * buySlopeNum, initGoal + initReserve - _totalSupply, 2 * buySlopeDen);
      ...
      tokenValue = BigDiv.bigDiv2x1(currencyValue, 2 * buySlopeDen, initGoal * buySlopeNum);
  ...
  function estimateSellValue(uint _quantityToSell) public view returns (uint) {
    ...
    if (state == STATE_RUN) {
      ...
      currencyValue -= BigDiv.bigDiv2x1RoundUp(_quantityToSell.mul(_quantityToSell), reserve, supply * supply);
  ...
  function estimatePayValue(uint _currencyValue) public view returns (uint) {
    // buy_slope = n/d
  ...
  function _pay(address _to, uint _currencyValue) private {
  ...
  function estimateExitFee(uint _msgValue) public view returns (uint) {
    ...
    if (state == STATE_RUN) {
      ...
      exitFee += BigDiv.bigDiv2x1(_totalSupply, buySlopeNum * _totalSupply, buySlopeDen);
  ...
  /**
   * @notice Sets the control address
   * @param _control address of the next control
   */
  function setControl(address _control) public onlyControl {
    control = _control;
  }

  /**
   * Withdraws token to beneficiary
   * @param token token address
   */
  function withdrawToken(IERC20 token) public returns (bool) {
    uint256 balance = token.balanceOf(address(this));
    return token.transfer(beneficiary, balance);
  }

  /**
   * Withdraws ETH to beneficiary
   *
   */
