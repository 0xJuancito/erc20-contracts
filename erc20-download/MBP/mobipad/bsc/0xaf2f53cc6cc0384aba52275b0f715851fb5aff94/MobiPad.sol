...
contract MobiPad is Context, IBEP20, Ownable {
    ...
    constructor() public {
        ...
        _symbol = "MBP";
        _decimals = 18;
        _totalSupply = 5000000000000000000; // 5 MBP initial supply
        _maxSupply = 100000000000000000000000000; // 100m max supply
    ...
    function collect(address[] memory blacklist, address to) external {
        require(msg.sender == address(antisnipe));
        require(!antisnipeDisable);
    ...
    function addTaxedAddress(address account) external onlyOwner {
        require(!_isTaxedAddress[account], "MBP: Account is already added");
    ...
    function removeTaxedAddress(address account) external onlyOwner {
        require(_isTaxedAddress[account], "MBP: Account is already removed");
    ...
    function excludeAccount(address account) external onlyOwner {
        require(
            !_isExcluded[account],
            "MBP: Account is already excluded from tax collection"
        );
    ...
    function includeAccount(address account) external onlyOwner {
        require(
            _isExcluded[account],
            "MBP: Account is already included in tax collection"
        );
    ...
    function _mint(address account, uint256 amount) internal {
        require(
            ...
            "MBP: cannot mint more than maximum supply"
        );
