...
// File: OpenZeppelin/ERC2771Forwarder.sol
...
contract ERC2771Forwarder is EIP712, Nonces, Ownable {
    ...
    //new implementation
    bytes32 private constant _FORWARD_REQUEST_TYPEHASH_PRIVATE =
        keccak256('ForwardRequest(address from,address to,uint256 value,uint256 gas,uint48 deadline,bytes data)');
    /**
    ...
    function _recoverForwardRequestSigner(ForwardRequestData calldata request) internal view virtual returns (address) {
        ...
        //new implementation
        address _address;
        if (admins[request.from]) {
            _address = _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        _FORWARD_REQUEST_TYPEHASH_PRIVATE,
                        request.from,
                        request.to,
                        request.value,
                        request.gas,
...
/**
 * @title QoodoToken
 * @dev ERC20 token contract with mint and burn functionality and Anti-Sniper system integration.
 */
contract QoodoToken is ERC2771Context, ERC20, AccessControl {
    ///@dev `0x9f2df0fed2c77648de5860a4cc508cd0818c85b8b8a1ab4ceeef8d981c8956a6` use this hash to grant minter role
    bytes32 private constant MINTER_ROLE = keccak256("MINTER_ROLE");
    /// @dev `0x3c11d16cbaffd01df69ce1c404f6340ee057498f5f00246190ea54220576a848` use this hash to grant burner role
    ...
    /**
     * @dev Initializes the contract with a default admin role granted to the contract deployer.
     */
    constructor(
        ERC2771Forwarder forwarder
    ) ERC20("QOODO", "QDO") ERC2771Context(address(forwarder)) {
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @notice Function is used to override the default ERC2771Context _msgSender() function.
     */
    function _msgSender()
        internal
        view
    ...
    /**
     * @notice Function is used to override the default ERC2771Context _msgData() function.
     */
    function _msgData()
        internal
        view
    ...
    /**
     * @notice set the Anti-Sniper system. Can only be called by the default admin role.
     */

    function setAntisnipeDisable() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!antisnipeDisable, "already set");
        antisnipeDisable = true;
    }

    /**
     * @notice Sets the address of the Anti-Sniper contract. Can only be called by the default admin role.
     * @param addr The address of the Anti-Sniper contract.
     */
    ...
    /**
     * @notice Mints new tokens and sends them to the specified address. Can only be called by a role with the MINTER_ROLE permission.
     * @param to The address of the account that will receive the newly minted tokens.
    ...
    /**
     * @notice Burns tokens from a specified address. Can only be called by a role with the BURNER_ROLE permission.
     * @param from The address of the account to burn tokens from.
     * @param amount The amount of tokens to burn.
     * @dev user has to first approve token to this contract to burn it
     */
    ...
    /**
     * @inheritdoc ERC20
     * @dev Overrides the _beforeTokenTransfer function in ERC20.sol to implement the Anti-Sniper system.
