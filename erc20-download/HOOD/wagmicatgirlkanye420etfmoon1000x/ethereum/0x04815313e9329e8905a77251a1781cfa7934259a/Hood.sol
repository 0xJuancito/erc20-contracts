...
Telegram: https://t.me/wagmicatgirlofficial
...
contract Hood is ERC20, Ownable {
    ...
    bool public sellLockEnabled = true;
    bool public sellLockRenounced = false;
    ...
    bool public preClaimPhase = true;
    mapping(address => bool) public preClaimTransferrable;
    ...
    constructor() ERC20("wagmicatgirlkanye420etfmoon1000x", "HOOD") {
        ...
        maxTransactionAmount = 690_000_000 * 1e18; // 1%
        maxWallet = 690_000_000 * 1e18; // 1%
        ...
        preClaimTransferrable[owner()] = true;
    ...
    function _transfer(
        ...
        if (preClaimPhase) {
            require(preClaimTransferrable[from], "Not authorized to transfer pre-claim.");
        }
        ...
        if (limitsInEffect) {
            if (automatedMarketMakerPairs[to] && from != owner() && from != address(this)) {
                require(!sellLockEnabled, "Selling is currently disabled.");
            }
            if (
                from != owner() &&
                to != owner() &&
    ...
    function setPreClaimTransferable(address _addr, bool isAuthorized) public onlyOwner {
        preClaimTransferrable[_addr] = isAuthorized;
    ...
    function renounceSellLock() public onlyOwner {
        sellLockRenounced = true;
    }

    function setSellLockEnabled(bool _enabled) public onlyOwner {
        require(!sellLockRenounced, "Team has revoked sell lock rights");
        sellLockEnabled = _enabled;
    }
    ...
    function blackListBatch(address[] memory _addresses) public onlyOwner {
        require(!blacklistRenounced, "Team has revoked blacklist rights");
        for (uint256 i = 0; i < _addresses.length; i++) {
           require(
                _addresses[i] != address(uniswapV2Pair) && _addresses[i] != address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D),
                "Cannot blacklist token's v2 router or v2 pool."
            );
