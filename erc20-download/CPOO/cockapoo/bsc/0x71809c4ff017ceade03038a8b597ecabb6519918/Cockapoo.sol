...
contract Cockapoo is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0x49aE6A2c75C14159D8C67C4Dca9846685dA3e617); // Marketing Address

    address payable public devAddress = payable(0x0Fa2a6f1058735261c67A1566d84D7e8427dD41D); // Dev Address
    ...
    string private constant _name = "Cockapoo";
    string private constant _symbol = "CPOO";
    ...
    constructor() {
        ...
        address futureOwner = address(0x4335bE29CB811D41ED760eA4Ce040a38EED07D50);
        
        liquidityAddress = payable(newOwner);
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                tokensToBurn = amount * _buyBurnFee / 100;
                _taxFee = _buyTaxFee;
            ...
            // Sell
                ...
                tokensToBurn = amount * _sellBurnFee / 100;
                _taxFee = _sellTaxFee;
        ...
        if(tokensToBurn > 0){
            _tokenTransfer(address(this), address(0xdead), tokensToBurn);
        }
        ...
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyDevFee + _buyBurnFee <= 20, "Must keep taxes below 20%");
    }
        ...
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellDevFee + _sellBurnFee <= 30, "Must keep taxes below 30%");
    }

    function setMarketingAddress(address _marketingAddress) external onlyOwner {
        require(_marketingAddress != address(0), "can't set to 0 address");
        marketingAddress = payable(_marketingAddress);
        _isExcludedFromFee[marketingAddress] = true;
    }
    
    function setLiquidityAddress(address _liquidityAddress) external onlyOwner {
        require(_liquidityAddress != address(0), "can't set to 0 address");
        liquidityAddress = payable(_liquidityAddress);
        _isExcludedFromFee[liquidityAddress] = true;
    }

    function setDevAddress(address _devAddress) external onlyOwner {
        require(_devAddress != address(0), "can't set to 0 address");
    ...
    function buyAndBurnTokens(uint256 bnbAmountInWei) external onlyOwner {
