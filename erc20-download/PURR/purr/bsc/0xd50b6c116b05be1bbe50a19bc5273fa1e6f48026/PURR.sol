...
contract PURR is Context, Ownable, IBEP20, BEP20Detailed {
  ...
  using SafeMath for uint256;
    ...
    uint256 public sellNFTStakingFee = 1;
    uint256 public buyNFTStakingFee = 0;
    uint256 private stakingPoolFee;
    bool public tradingEnabled = false;
    uint256 public startTradingBlock;
  
    address payable public stakingPoolAddress = payable(0x56EC8695c18192767b4a5617D1426A70367e7593);
  
    bool inSwapAndLiquify;
    bool private swapAndLiquifyEnabled = true;

    uint256 public numTokensSellToFee = 1000000 * 10**18;
    ...
    constructor () BEP20Detailed("PURR", "PURR", 18) {
      _owner = msg.sender ;
    ...
    function setBuyNFTStakingFeePercent(uint256 updatedBuyNFTStakingFee) external onlyOwner {
        require(updatedBuyNFTStakingFee <= 3, "Buy fee should be less than 3%");
        buyNFTStakingFee = updatedBuyNFTStakingFee;
     
    }

    function setSellNFTStakingFeePercent(uint256 updatedSellNFTStakingFee) external onlyOwner {
        require(updatedSellNFTStakingFee <= 3, "Sell fee should be less than 3%");
        sellNFTStakingFee = updatedSellNFTStakingFee;
     
    }

    function setNFTStakingAddress(address payable wallet) external onlyOwner {
        require(wallet != stakingPoolAddress,"NFTStaking address is already that address");
        require(wallet != address(0), "NFTStaking address cannot be dead address");
    ...
    function changeNumTokensSellToFee(uint256 _numTokensSellToFee) external onlyOwner {
        require(_numTokensSellToFee >= 90000 * 10**18 && _numTokensSellToFee <= 90000000 * 10**18, "Swap to fee threshold must be set within 90,000 to 90,000,000 tokens");
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        ...
        if(takeFee)
        {
            if(sender == uniswapV2Pair){stakingPoolFee = buyNFTStakingFee;}
            else{stakingPoolFee = sellNFTStakingFee;}
            uint256 taxAmount = amount.mul(stakingPoolFee).div(100);
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(contractTokenBalance); // <- this breaks the ETH -> wDoge swap when swap+liquify is triggered

        payable(stakingPoolAddress).transfer(address(this).balance);
