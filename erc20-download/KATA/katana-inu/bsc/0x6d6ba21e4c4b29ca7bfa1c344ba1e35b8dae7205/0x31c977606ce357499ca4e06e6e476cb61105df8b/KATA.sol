...
contract KATA is IERC20, OwnableUpgradeSafe, LGEWhitelisted {
    ...
    bool public _feesLocked;

	uint256 public _feeTokenPct;
	uint256 public _feeTokenToPct;
	...
	address[] public _feeTokenToSwapPath;

    mapping(address => bool) public _excluded;
    
    function initialize(uint256 feeTokenPct, uint256 feeTokenToPct, address feeAddress, address router)
        public
        initializer
    {
        ...
        address[] memory feeTokenToSwapPath = new address[](2);
            
        feeTokenToSwapPath[0] = address(this);
        feeTokenToSwapPath[1] = r.WETH();
		
		setFees(feeTokenPct, feeTokenToPct, feeTokenToSwapPath);
    ...
    function setFeesLocked() public onlyOwner {
        require(!_feesLocked, "Fees are locked already");
        _feesLocked = true;
    }
    
    function setFees(uint256 feeTokenPct, uint256 feeTokenToPct, address[] memory feeTokenToSwapPath) public onlyOwner {
        require(!_feesLocked, "Fees are locked");
        require(feeTokenPct.add(feeTokenToPct) <= 10000, "Fees must not total more than 100%");
        require(feeTokenToSwapPath.length > 1, "Invalid path");
		
		
		_feeTokenPct = feeTokenPct;
		_feeTokenToPct = feeTokenToPct;
		_feeTokenToSwapPath = feeTokenToSwapPath;
		
		
    }

    function setFeeAddress(address feeAddress) public onlyOwner {
        require(feeAddress != address(0), "Fee reward address must not be zero address");
	...
	function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if(_pair[recipient] && !_feeExcluded[sender]) {
			...
			if(_feeTokenPct > 0 && _feeAddress != address(0)) {
			
				feeTokenAmount = amount.mul(_feeTokenPct).div(10000);
				
				_balances[_feeAddress] = _balances[_feeAddress].add(feeTokenAmount);
				emit Transfer(sender, _feeAddress, feeTokenAmount);
				
			}
			
			uint256 feeTokenToAmount = 0;
			
			if(_feeTokenToPct > 0 && _feeAddress != address(0))  {
			    
				feeTokenToAmount = amount.mul(_feeTokenToPct).div(10000);
				
				if(_router != address(0)) {
				    
    				_balances[address(this)] = _balances[address(this)].add(feeTokenToAmount);
    				
    				emit Transfer(sender, address(this), feeTokenToAmount);
    				
    				IUniswapV2Router02 r = IUniswapV2Router02(_router);
                    
                    _approve(address(this), _router, feeTokenToAmount);
				    ...
				    _balances[_feeAddress] = _balances[_feeAddress].add(feeTokenToAmount);
				    emit Transfer(sender, _feeAddress, feeTokenToAmount);
				}
				
			}
			
			amount = amount.sub(feeTokenAmount).sub(feeTokenToAmount);
