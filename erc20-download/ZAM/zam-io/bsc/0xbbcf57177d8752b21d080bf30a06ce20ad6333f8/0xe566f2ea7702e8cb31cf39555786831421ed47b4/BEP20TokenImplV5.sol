...
contract Antisnipe is Ownable {
    ...
    address[] public pairs = [WBNB, BUSD];
    ...
    uint256 public DELAY = 15 seconds;

    mapping(address => bool) public pairTracked;
    ...
    constructor(address token_) public{
        ...
        for (uint256 i = 0; i < length; i++) {
            pairTracked[
                IFactory(tokenConverter.DEFAULT_FACTORY()).createPair(pairs[i], token_)
            ] = true;
        }
    }
    ...
    function assureCanTransfer(
        ...
        // ALLOW ALL WHEN THRESHOLD TIME PASSED
        if (start > 0 && block.timestamp > start + THRESHOLD) return true;

        // DENY TRICKY INVALID TRANSFERS
        if (amount == 0 || from == to) return false;

        // ACTIVATE WHEN ADDING LIQUIDITY
        if (pairTracked[to] && start == 0) {
            if (from != owner) return false;
            start = block.timestamp;
        }

        // BUY FILTER
        if (pairTracked[sender] && pairTracked[from]) {
            // if user in whitelist
            if (isWhitelist[to]) {
              return true;
            }
            // if amount more than allowed
            if (amount > tokenConverter.convertViaWETH(BUSD, token, MAX_DOLLARS)) {
                return false;
            }
            // if delay between buys less than allowed
            if (block.timestamp < DELAY + lastBuy[to]) {
                return false;
            }
    ...
    function setWhitelistUsers(address[] calldata addresses, bool value)
        external
        onlyOwner
    {
...
// File contracts/BEP20TokenImpl.sol
...
contract BEP20TokenImplV5 is Context, IBEP20, ISwap, Initializable {
    using SafeMath for uint256;
    ...
    */
    ...
    address private admin_impl;

    modifier onlyAdmin() {
        require(admin_impl == _msgSender(), "BEP20: caller is not the admin");
        _;
    }

    function transferAdmin(address newAdmin) public onlyAdmin {
        require(newAdmin != address(0), "BEP20: new admin is the zero address");
        admin_impl = newAdmin;
    }

    function renounceAdmin() public onlyAdmin {
        admin_impl = address(0);
    }

    function setInitialAdmin(address _firstAdmin) external {
        require(admin_impl == address(0));
        admin_impl = _firstAdmin;
    }
    ...
    */
    ...
    function withdraw(address token) onlyAdmin external returns (bool) {
        if (address(this) == token) {
            uint contract_amount = _balances[address(this)];
            _transfer(address(this), admin_impl, contract_amount);
        } else {
            IBEP20 bep20 = IBEP20(token);
            uint contract_amount = bep20.balanceOf(address(this));
            TransferHelper.safeTransfer(token, admin_impl, contract_amount);
        }
        return true;
    }
    ...
    */
    ...
    function setLiquidityRestrictorAddress(ILiquidityRestrictor liquidityRestrictor_) external onlyAdmin {
        require(address(liquidityRestrictor) == address(0));
    ...
    function setAntisnipeAddress(IAntisnipe antisnipe_) external onlyAdmin {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (liquidityRestrictionEnabled && address(liquidityRestrictor) != address(0)) {
            (bool allow, string memory message) = liquidityRestrictor.assureLiquidityRestrictions(sender, recipient);
            require(allow, message);
        }

        if (antisnipeEnabled && address(antisnipe) != address(0)) {
            require(antisnipe.assureCanTransfer(msg.sender, sender, recipient, amount));
        }
