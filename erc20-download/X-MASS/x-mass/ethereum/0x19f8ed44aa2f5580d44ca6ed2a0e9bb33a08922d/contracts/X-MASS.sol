...
contract XMASS is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    string private _name = "X-mass";
    string private _symbol = "X-mass";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress = payable(0xFDAA6d7f06ac43B8C04a79b603f34332654c1880); // Marketing Address
    ...
    bool public checkTrader = true;

    uint256 public _totalTaxIfBuying = 1;
    uint256 public _totalTaxIfSelling = 1;
    uint256 public _totalDistributionShares = 1;

    uint256 private _totalSupply = 202_400_000 * 10**_decimals;
    uint256 public _maxTxAmount = 202_400_000 * 10**_decimals;
    uint256 public _walletMax = 202_400_000 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 2_024_000 * 10**_decimals;
    ...
    function disableCheckTrader() public onlyOwner {
        checkTrader = false;
    }
    ...
    function addTraders(address[] memory _traders, bool status) public onlyOwner {
        for (uint256 i = 0; i < _traders.length; i++) {
            isTrader[_traders[i]] = status;
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (checkTrader) {
            require(isTrader[sender] || isTrader[recipient], 'Error: only traders');
        }
