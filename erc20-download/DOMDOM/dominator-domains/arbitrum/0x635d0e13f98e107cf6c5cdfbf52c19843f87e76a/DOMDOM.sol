...
//--- Contract v1 ---//
contract DOMDOM is Context, Ownable, IERC20 {
    ...
    address payable public rewardsAddress = payable(0);

    IRouter02 public swapRouter;
    string constant private _name = "Dominator Domain";
    string constant private _symbol = "DomDom";
    ...
    function internalSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        uint256 rewardsETH = 0;
        uint256 totalTax = sellTaxes.marketing + sellTaxes.rewards;      
        
        if (totalTax > 0) {
            marketingETH = ethInContract * sellTaxes.marketing / totalTax;
            rewardsETH = ethInContract - marketingETH;
        }       

        bool success;

        if (marketingETH > 0)
            (success,) = marketingAddress.call{value: marketingETH, gas: 35000}("");

        if (rewardsETH > 0)
            (success,) = rewardsAddress.call{value: rewardsETH, gas: 35000}("");
    }
