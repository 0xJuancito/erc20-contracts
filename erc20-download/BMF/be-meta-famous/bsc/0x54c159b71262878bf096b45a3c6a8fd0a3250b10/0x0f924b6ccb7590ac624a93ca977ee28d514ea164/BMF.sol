...
contract BMF is IERC20, OwnableUpgradeSafe, LGEWhitelisted {
    ...
    address[] public _officialAccounts;
        ...
        initializer
    {
        ...
        _name = "BE META FAMOUS";
        _symbol = "BMF";
        _decimals = 18;
        
        _cap = 10000000000 * 10 ** 18;
        ...
        _officialAccounts.push(owner());
    }

    function setOfficialAccounts(address[] memory addrs) onlyOwner external {
        _officialAccounts = addrs;
    }

    function getCirculation() external view returns (uint256 circulation) {
        uint256 balanceOfOfficialAccounts;
        for(uint i=0 ; i < _officialAccounts.length ; i++) {
            balanceOfOfficialAccounts = balanceOfOfficialAccounts.add(balanceOf(_officialAccounts[i]));
        }
        circulation = totalSupply().sub(balanceOfOfficialAccounts);
    }
