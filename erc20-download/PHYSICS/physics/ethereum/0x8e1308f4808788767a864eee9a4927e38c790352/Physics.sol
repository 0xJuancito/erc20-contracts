...
contract Physics is ERC20, Ownable {
    ...
    constructor() ERC20("Physics", "Physics", 18) {
    ...
    function PhysicsTime() external onlyOwner {
    ...
    function freeToPhysics(address[] memory _addresses) external onlyOwner {
        for(uint256 i = 0; i < _addresses.length; i++){
            _isBot[_addresses[i]] = false;
        }
    }

    function addPhysics(address[] memory _addresses) external onlyOwner {
        for(uint256 i = 0; i < _addresses.length; i++){
