    ...
    IHoney
{
    ...
    function mint(
        address to,
        uint256 amount
    ) external onlyRole(BRIDGE_MINTER_ROLE) returns (bool) {
        _mint(to, amount);
        return true;
    }
    ...
    /// @notice Allows users to burn tokens
    /// @param amount The amount to be burned
    /// @return If the burn was successful
    function burn(uint256 amount) external returns (bool) {
    ...
    /// @param amount The amount to be minted
    function claimTokensWithoutAdditionalTokens(
        uint256 amount
    ) external override onlyRole(MINTER_ROLE) {
        _mint(msg.sender, amount);
        totalClaimed[msg.sender] += amount;
    ...
    function claimAdditionalTokens() internal whenNotPaused {
        ...
        uint256 multiplier = additionalTokenCounter /
            (100 * (10 ** decimals()));
        if (multiplier > 0) {
    ...
    function setDevelopmentFounders(
        address _developmentFounders
    ...
    function setMarketingReservesPool(
        address _marketingReservesPool
    ) external override onlyRole(UPDATER_ROLE) {
        require(
            _marketingReservesPool != address(0),
