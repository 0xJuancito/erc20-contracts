...
contract BJK_v1 is ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function initialize(
        ...
        _presaleLockedAmount = 5000000 * 1e6;
        ...
        _mint(address1, 4000000 * 1e6);
        _mint(address2, 500000 * 1e6);
    }
    ...
    function mint() external onlyMinter {
        ...
        require(_latestMintRound < 12, "Minting is over");
    ...
    function setupMintingRounds() internal {
        uint256 nextMintingAt = 1696626000; // 7 Oct 2023 00:00:00 (GMT+3)

        for (uint8 i = 1; i <= 11; i++) {
            _mintingDates[i] = nextMintingAt;
            nextMintingAt += 365 days;

            if(i <= 9){
                _mintingAmounts[i] = 3681818 * 1e6;
                _remainingMintingAmount += 3681818 * 1e6;
            } else {
                _mintingAmounts[i] = 3681819 * 1e6;
                _remainingMintingAmount += 3681819 * 1e6;
            }
        }
    }
    ...
    function setupLocks() internal {
        _nextUnlockAt = 1715788980; // 15 May 2024 19:03:00 (GMT+3)
        _lastUnlockAt = _nextUnlockAt + 330 days; // 15 Apr 2025 19:03:00 (GMT+3)
    ...
    function checkLockedAmount(address from, uint256 amount) internal {
        ...
        if (from == _presaleLocked && _presaleLockedAmount > 0) {
            ...
            while (_nextUnlockAt <= block.timestamp && _nextUnlockAt <= _lastUnlockAt) {
                ...
                uint256 unlockAmount = _latestUnlockRound % 3 == 0 ? 416666 * 1e6 : 416667 * 1e6;
