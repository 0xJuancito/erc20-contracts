...
/**
 * @dev The official HBIT smart contract
 */
contract HashBit is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    // General Info
    string private _name     = "HashBit 2.0";
    string private _symbol   = "HBIT";
    uint8  private _decimals = 18;
    
    // Liquidity Settings
    IUniswapV2Router02 public _pancakeswapV2Router; // The address of the PancakeSwap V2 Router
    address public _pancakeswapV2LiquidityPair;     // The address of the PancakeSwap V2 liquidity pairing for HBIT/WBNB
    ...
    // Addresses
    address payable public _marketingAddress = payable(0x161F252635fbCbb5fA9b3cF5E5886Ad3c18858C8); // Marketing address used to pay for marketing
    ...
    // Token Tax Settings
    uint256 public  _taxFee           = 0;  // 0% tax 
    uint256 public  _sellTaxFee       = 3; // 3% tax 
    uint256 public  _whaleSellTaxFee  = 10; // 10% tax 
    uint256 private _previousTaxFee;
    
    // Token Limits
    uint256 public _maxTxAmount        = 50000000000 * 10**18; // ===========
    uint256 public _tokenSwapThreshold = 500000 * 10**18; // ===========
    
    // Timer Constants 
    uint private constant DAY = 60 * 60 * 24; // How many seconds in a day
    
    // Anti-Whale Settings 
    uint256 public _whaleSellThreshold = 50000000 * 10**18; // ============
    ...
    uint256 public _buybackUpperLimit    = 1000000 * 10**18; // 1000000 $HBIT will be used to buy at maximum in any one trade
    ...
    uint256 public  _lotteryThreshold    = 500000 * 10**18; // 500000 tokens required to be in the pool before lottery can be triggered
    uint256 public  _lotteryMinimumSpend = 500000 * 10**18; // 500000 tokens required to buy before before lottery can be triggered
    ...
    /**
     * @notice Withdraws non-HBIT tokens that are stuck as to not interfere with the liquidity
     */
    ...
    function random() private returns (uint) {
        uint r = uint(uint256(keccak256(abi.encodePacked(block.prevrandao, block.timestamp, _nonce))) % 1000);
        r = r.add(1);
        _nonce++;
        return r;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // Gets the contracts HBIT balance for buybacks, charity, liquidity and marketing
    ...
    /**
     * @notice Burns HBIT tokens straight to the burn address
     */
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        // Swap 7/8ths of HBIT tokens for BNB
        ...
        // We now have 6/7ths left of BNB (converted from HBIT)
