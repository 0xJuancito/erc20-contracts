// File: contracts/NewToken/IUniswapV2Pair.sol
...
// File: contracts/NewToken/IUniswapV2Factory.sol
...
// File: contracts/NewToken/IUniswapV2Router02.sol
...
// File: contracts/NewToken/YYToken.sol
...
contract RatToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public vaultFees = 10;
    uint256 public liqFees = 10;
    ...
    constructor(address _router, address _vaultAddress, address _devAddress, address recipient) public ERC20("ratsDAO", "rat") {
        ...
        _mint(recipient, 500000000000 * (10**18)); 
    }
    ...
    function setDevFees(uint256 value) external onlyOwner{
        require(value <= 300 && value > 1, "Over max fee");
        devFees = value;
        require(devFees + vaultFees + liqFees <= 300, "Over max fee");
    }
    function setVaultFees(uint256 value) external onlyOwner{
        require(value <= 300 && value > 1, "Over max fee");
        vaultFees = value;
        require(devFees + vaultFees + liqFees <= 300, "Over max fee");
    }
    function setLiqFees(uint256 value) external onlyOwner{
        require(value <= 300 && value > 1, "Over max fee");
        liqFees = value;
        require(devFees + vaultFees + liqFees <= 300, "Over max fee");
    }
    ...
    function _transfer(
        ...
        bool takeFee = false;
        if( !swapping && (automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to])) {
            takeFee = true;
        }
        ...
        uint256 totalFees = (devFees + vaultFees + liqFees);
    ...
    function processFee(uint256 contractTokenBalance) private {
        ...
        uint256 totalFees = (devFees + vaultFees + liqFees);
        uint256 pairTotal = (totalFees - liqFees.div(2));

        uint256 convertAmount = contractTokenBalance;
        uint256 ethBal = address(this).balance;
        swapTokensForEth(convertAmount.mul(pairTotal).div(totalFees));
        uint256 ethOut = address(this).balance - ethBal;

        payable(devAddress).transfer(ethOut.mul(devFees).div(pairTotal));
        payable(vaultAddress).transfer(ethOut.mul(vaultFees).div(pairTotal));
