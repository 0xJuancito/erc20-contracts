...
interface IDiamondClawElite {
  function isHolderActive(address account) external view returns (bool);

  function isHolderActivePremium(address account) external view returns (bool);
...
contract CakeMonster is ERC20SnapshotUpgradeableCustom, OwnableUpgradeable {
  ...
  // V17 - new cycle end
  event ClaimNewMonsta(address indexed sender, uint256 indexed amount);
  uint256 public newCycleSnapshotId;
  mapping(address => uint256) public newMonstaClaims;

  // V18 DCE upgrades
  address public ADDY_DCE;
  uint256 public dceSupplySnapshot;
  ...
  function setDceAddy(address _addy) public onlyOwner {
    ADDY_DCE = _addy;
  }

  function config() public onlyOwner {
    SWAP_ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    SWAP_FACTORY = 0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73;
  }
  ...
  function iTax(address target) external {
    require(!finished, 'FINISHED');
    require(!_isWhitelisted(target), 'TARGET_WHITELISTED');

    // 4320000 seconds = 50 days for default holders, 100 days (8640000 seconds) for active DCE premium holders
    if (IDiamondClawElite(ADDY_DCE).isHolderActivePremium(target)) {
      require(block.timestamp - last5pctMove[target] > 8640000, '5PCT_MOVE_MADE');
    } else {
    ...
    // 2. Burn 15% (vault addition) + 15% (deflationary burn) of target wallet
    uint256 burnAmountTarget = _pct(3000, balance); // 30.0% (15% + 15%)
  ...
  function kitchenMake() external {
    ...
    if (totalSupply() < nextInterimClaimThreshold && block.timestamp > startTime + 14 days) {
      ...
      dceSupplySnapshot = IDiamondClawElite(ADDY_DCE).totalSupply();
    }
  ...
  function endCurrentCycle() external {
  ...
  function claimSlice() external {
    ...
    last5pctMove[_msgSender()] = block.timestamp;

    emit Claim(_msgSender());
  }

  function claimCrumbs() external {
    require(!finished, 'FINISHED');
    ...
    // Only active DCE holders can claim crumbs
    if (dceSupplySnapshot > 0) {
      require(IDiamondClawElite(ADDY_DCE).isHolderActive(_msgSender()), 'ACTIVE_DCE_REQUIRED');
    }
    ...
    uint256 baseSharePct = 0;

    if (dceSupplySnapshot > 0) {
      // Equal share for all DCE holders
      baseSharePct = (1 ether) / dceSupplySnapshot;
    } else {
      baseSharePct =
        (callerBalance * 1 ether) /
  ...
  function claimNewMonsta() external {
    require(!finished, 'FINISHED');
    require(block.timestamp <= startTime + 14 days, 'NEW_MONSTA_CLAIM_ENDED');
    require(newCycleSnapshotId > 0, 'NO_CLAIM');

    require(newMonstaClaims[_msgSender()] < newCycleSnapshotId, 'ALREADY_CLAIMED');

    uint256 callerBalance = balanceOfAt(_msgSender(), newCycleSnapshotId);

    require(callerBalance > 0, 'EMPTY_WALLET_SNAPSHOT');

    // Track caller claimed
    newMonstaClaims[_msgSender()] = newCycleSnapshotId;

    // Calc share of total supply in %
    uint256 baseSharePct = (callerBalance * 1 ether) / totalSupplyAt(newCycleSnapshotId);

    uint256 newCallerBalance = (initialSupply * baseSharePct) / 1 ether;
    uint256 mintAmount = newCallerBalance - callerBalance;
    ...
    emit ClaimNewMonsta(_msgSender(), mintAmount);
  }

  function startNewCycle() external {
    require(finished, 'NOT_FINISHED');
    require(block.timestamp - finishTime > 2592000, 'CLAIM_NOT_FINISHED');
  ...
  function _transferHelper(
    ...
    // Prevent flash buys/sells on cycle end
    if (finished) {
      if (block.timestamp - finishTime < 2 minutes) {
        require(false, 'COOLDOWN');
        return false;
      }

      if (
        block.timestamp - finishTime < 30 minutes &&
        last5pctMove[sender] > finishTime - 10 minutes &&
        last5pctMove[sender] <= finishTime &&
        !_isLP(sender)
      ) {
        require(false, 'COOLDOWN');
        return false;
      ...
      // Prevent flash buys/sells on cycle start
      ...
      if (
        block.timestamp - startTime < 30 minutes &&
        last5pctMove[sender] > startTime - 10 minutes &&
        last5pctMove[sender] <= startTime &&
        !_isLP(sender)
      ) {
        require(false, 'COOLDOWN');
        return false;
    ...
    // Skip tax for buys if holder has active premium DCE
    if (IDiamondClawElite(ADDY_DCE).isHolderActivePremium(recipient) && _isLP(sender)) {
      _transfer(sender, recipient, amount);
  ...
  function _swapToEth(address _tokenIn, uint256 _amountIn, address _to) private {
