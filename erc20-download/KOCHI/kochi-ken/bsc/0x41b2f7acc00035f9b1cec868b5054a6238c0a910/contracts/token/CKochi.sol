...
contract Kochi is Context, ERC20, ERC20Burnable, Ownable, Pausable, ReentrancyGuard {
  ...
  uint256 public taxThreshold = 420000 * 10 ** 18; // threshold before the tax is applied; this allows small transaction to not pay the gas for the tax swaps
  ...
  function payTaxes(uint256 amount, uint256 total_taxed_amount, bool buyer, address from) private nonReentrant returns (uint256 remainder) {
    ...
    console.log("amountOut", amountOut > autoLPminBaseTokenOut);
    if (amountOut > autoLPminBaseTokenOut) _resolveTaxes();
  ...
  function _resolveTaxes() private {
  ...
  function resolveTaxes() external nonReentrant {
    _resolveTaxes();
  }
