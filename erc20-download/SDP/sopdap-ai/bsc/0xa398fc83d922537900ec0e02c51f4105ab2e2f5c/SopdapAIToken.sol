...
contract SopdapAIToken is ERC20 {
    ...
    // This smart contract is developed by Sopdap Technologies ai.sopdap.com SOPDAP AI (SDP)
    constructor() ERC20("SOPDAP AI", "SDP") {
        _owner = msg.sender;
        uint256 totalSupply = 3_000_000_000 * 10**decimals();      
        _treasuryLocked = totalSupply * 25 / 100;
        _devLocked = totalSupply * 25 / 100;
    ...
    // token lockupAirdrop.
    function lockupAirdrop(address[] memory recipients, uint256[] memory amounts, uint256 lockupPeriod) public returns (bool) {
...
// function to get locked balance of a recipient
    ...
    function lockedSDPTreasury() external {
        require(_msgSender() == _owner, "SOPDAP AI: only owner can lock treasury");
        _transfer(_owner, address(this), _treasuryLocked);
    }

    function lockedSDPToken() external {
        require(_msgSender() == _owner, "SOPDAP AI: only owner can lock SDP");
    ...
    function releaseSOPToken() external {
        require(_msgSender() == _owner, "SOPDAP AI: only owner can release Major Token Loocked");
    ...
    function lockSDPTokens(address account, uint256 amount, uint256 duration) public {
    ...
    function unlockSDPTokens(address account, uint256 amount) public {
