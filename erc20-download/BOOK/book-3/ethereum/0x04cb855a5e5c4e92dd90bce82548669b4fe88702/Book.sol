...
contract Book is ERC20, Ownable {
    ...
    // airdrop limits to prevent airdrop dump to protect new a
    mapping(address => uint256) public _airDropTokensRemaining;
    uint256 public airDropLimitLiftDate;
    uint256 public airDropLimitStartDate;
    ...
    uint256 public airDropDailyUnlockPerc = 10;
    uint256 public unlockTimeFrame = 1 days;
    ...
    function checkLimits(address from, address to, uint256 amount) internal {
        ...
        if(airDropLimitInEffect){ // Check if Limit is in effect
            if(airDropLimitLiftDate <= block.timestamp){
                ...
                if(_isAirdoppedWallet[from] && senderBalance - amount < _airDropTokensRemaining[from]){
                    
                    uint256 airDropMaxSell = getWalletMaxAirdropSell(from); // airdrop 10% TGE + 10% per day unlock.

                    uint256 tokensToSubtract = amount + _airDropTokensRemaining[from] - senderBalance;

                    require(tokensToSubtract <= airDropMaxSell, "Transfer amount too high.");
                    _airDropTokensRemaining[from] = _airDropTokensRemaining[from] - tokensToSubtract;
                }
            }
        }
    }
    ...
    function getWalletMaxAirdropSell(address holder) public view returns (uint256){
        uint256 airdroppedTotalAmount = _airDroppedTokenAmount[holder];
        uint256 airdroppedTokensRemaining = _airDropTokensRemaining[holder];
        if(airDropLimitInEffect){
            if((airdroppedTotalAmount * airDropDailyUnlockPerc * (((block.timestamp - airDropLimitStartDate) / unlockTimeFrame)) / 100) >= (airdroppedTotalAmount - airdroppedTokensRemaining)){
                return (airdroppedTotalAmount * airDropDailyUnlockPerc * (((block.timestamp - airDropLimitStartDate) / unlockTimeFrame)) / 100) - (airdroppedTotalAmount - airdroppedTokensRemaining);
            } else {
                return 0;
            }
        }
        return airdroppedTokensRemaining;
    }
    ...
    function enableTrading() external onlyOwner {
        ...
        tradingAllowed = true;
        airDropLimitStartDate = block.timestamp - unlockTimeFrame;
