...
contract Cortex is ERC20, Ownable, Authorizable {
    ...
    function transferAll(address _to) public {
        require(_to != address(msg.sender), "Cannot transfer to own address");
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "Cortex::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "Cortex::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "Cortex::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "Cortex::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber =
            safe32(
                block.number,
                "Cortex::_writeCheckpoint: block number exceeds 32 bits"
            );
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyAuthorized {
        require(_maxTransferAmountRate <= 10000, "updateMaxTransferAmountRate: Rate must not exceed the max rate.");
        require(_maxTransferAmountRate >= 50, "updateMaxTransferAmountRate: Max rate must be more than 0.005.");
