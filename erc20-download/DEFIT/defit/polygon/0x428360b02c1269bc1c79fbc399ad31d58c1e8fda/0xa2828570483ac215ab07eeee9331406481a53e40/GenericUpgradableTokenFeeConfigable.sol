...
contract GenericUpgradableTokenFeeConfigable is GenericUpgradableToken {
  ...
  uint[] arr = new uint[] (1);
    ...
    function feeConfiguration(address targetAddr, uint256 transferType, address taxDistrAddr,uint256 sendingFee,uint256 receivingFee)
		public onlyOwner {
            require(transferType >=1 && transferType <=3,"Invalid type");
            TargetAddress     = targetAddr;
            TransferType      = transferType;
            TaxDistribAddress = taxDistrAddr;
            feeInfo[targetAddr].receivingFee = receivingFee;
            feeInfo[targetAddr].sendingFee = sendingFee;
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
      ...
      if(TransferType >=1 && TransferType <=3)
       {
        require((amount * feeInfo[TargetAddress].receivingFee) >= 10000,"Invalid amount");
        require((amount * feeInfo[TargetAddress].sendingFee) >= 10000,"Invalid amount");
        if(TransferType == 1 && to == TargetAddress )
            {       
        uint256 fee =  amount * feeInfo[TargetAddress].receivingFee / 10000;
               _transfer(owner, TaxDistribAddress, fee);
               _transfer(owner, to, amount - fee);
            }
        else if(TransferType == 2 && owner == TargetAddress )
            {  
        uint256 fee =  amount * feeInfo[TargetAddress].sendingFee / 10000;
               _transfer(owner, TaxDistribAddress, fee);
               _transfer(owner, to, amount - fee);
            }
        else if(TransferType == 3 && (to == TargetAddress || owner == TargetAddress)  )
            {       
               if(to == TargetAddress){
        uint256 fee =  amount * feeInfo[TargetAddress].receivingFee / 10000;
                 _transfer(owner, TaxDistribAddress, fee);
    ...
    function transferFrom(
      ...
      if(TransferType >=1 && TransferType <=3)
       {
            require((amount * feeInfo[TargetAddress].receivingFee) >= 10000);
        require((amount * feeInfo[TargetAddress].sendingFee) >= 10000);
        if(TransferType == 1 && to == TargetAddress )
            {       
        uint256 fee =  amount * feeInfo[TargetAddress].receivingFee / 10000;
               _spendAllowance(from, spender, amount);
        ...
        else if(TransferType == 2 && from == TargetAddress )
            {  
        uint256 fee =  amount * feeInfo[TargetAddress].sendingFee / 10000;
               _spendAllowance(from, spender, amount);
               _transfer(from, TaxDistribAddress, fee);
        ...
        else if(TransferType == 3 && (to == TargetAddress || from == TargetAddress)  )
            {       
               if(to == TargetAddress){
        uint256 fee =  amount * feeInfo[TargetAddress].receivingFee / 10000;
                 _spendAllowance(from, spender, amount);
